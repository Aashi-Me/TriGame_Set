...
class Rectangle(_BBox):
    ...
    def __init__(self, p1, p2, r=25):
        _BBox.__init__(self, p1, p2)

    def __repr__(self):
        return "Rectangle({}, {})".format(str(self.p1), str(self.p2))
    
    def _draw(self, canvas, options, r=50):
        self._reconfig("outline", None)
        p1 = self.p1
        p2 = self.p2
    ...
    def checkClickedOn(self, canvas):
        click = canvas.checkMouse()
        if click != None:
           x = click.getX()
           y = click.getY()
           p1 = self.p1
...
class Text(GraphicsObject):
    ...
    def __init__(self, p, text, ang=0):
    ...
    def _draw(self, canvas, options):
        p = self.anchor
        x,y = canvas.toScreen(p.x,p.y)
        return canvas.create_text((x,y),options,angle=self.config['angle'])
    ...
    def setAngle(self, ang):
        self._reconfig("angle", ang)
        
    def setJustify(self, just):
        self._reconfig("justify", just)
    ...
    def setFace(self, face):
        if face in ['helvetica','arial','courier','times roman','video-ezy',
                    'playfair display black', 'poppins', 'kunstler script']:
...
class Entry(GraphicsObject):
    ...
    def __init__(self, p, width):
        ...
        self.text.set("")
        self.fill = "white" #color_rgb(238, 250, 252)
        self.color = "black" #color_rgb(0, 20, 41)
